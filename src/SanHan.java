...
public class SanHan extends Player {
		...
		while (j < 20) { // 右下になるまでは探索する
			if (box[j] > 0 && box[j] < board_size * board_size) {
				if (board[box[j]] == id) { // 自分の石が置かれていたら
					...
					if (pos + (board_size + board_size) < (board_size * board_size)) { // 間置く ボ超
						if (board[pos + (board_size + board_size)] == id) { // 自コマか
							if (board[pos + board_size] == -1) { // 空白か
								if (put4(pos + board_size) == 1) {
									box[turns - 1] = pos + board_size;
									return (pos + board_size); // 三目置く
					...
					if ((pos + 1) < board_size * board_size) { // ボ超
						if (board[pos + 1] == id && (pos + 1) % board_size != (board_size - 1)) { // 自コマ&飛石阻止
							// 左置く時，ボ超
							if (pos - 1 >= 0) {
						...
						// 右斜め下\
						if ((pos + board_size + 1) < (board_size * board_size) && (pos + board_size + 1) > -1
								&& (pos + board_size + 1) % board_size != 0) { // ボ超&飛石阻止
							if (board[pos + board_size + 1] == id) { // 自コマか

								// ボ超&飛石阻止 右下
								if ((pos + (2 * (board_size + 1))) < (board_size * board_size) && (pos + (2 * (board_size + 1))) > -1
								...
								// ボ超&飛石阻止 左上
								if (pos - (board_size + 1) > -1 &&
										pos - (board_size + 1) > (board_size * board_size)
										&& (pos - (board_size + 1)) % board_size != (board_size - 1)) {
						...
						if (pos + (2 * (board_size + 1)) < board_size * board_size
								&& pos + (2 * (board_size + 1)) > -1
								&& (pos + 2 * (board_size + 1)) % board_size != 0) { // 間置くボ超飛石
						...
						if ((pos + board_size - 1) < board_size * board_size && (pos + board_size - 1) > -1
							...
							if (board[pos + board_size - 1] == id) { // 自コマか
								...
								// ボ超&飛石阻止 右上
								if (pos - (board_size - 1) > -1 &&
										pos - (board_size - 1) < (board_size * board_size) && (pos - (board_size - 1)) % board_size != 0) {
					...
					if ((pos + 1) < board_size * board_size && (pos + 1) % board_size != 0) { // 右
						if ((pos + 2) < board_size * board_size && (pos + 2) % board_size != 0) { // 三目狙い1つめ
							if ((pos - 1) > -1 && (pos - 1) % board_size != (board_size - 1)) { // 三目狙い2つめ
								if (board[pos + 1] == -1 && board[pos + 2] == -1 && board[pos - 1] == -1) {
									// 空白
									if (put4(pos + 1) == 1) {
										box[turns - 1] = pos + 1;
										return (pos + 1); // 三目置く
		...
		while (j < 20) { // 右下になるまでは探索する
			if (box[j] >= 0 && box[j] < board_size * board_size) {
				if (board[box[j]] == id) { // 自分の石が置かれていたら
					...
					if ((pos + 1) < board_size * board_size && (pos + 1) % board_size != 0) { // 右
						if (((pos + 2) < board_size * board_size && (pos + 2) % board_size != 0)) { // 三目狙いどちらか1つ
							if (board[pos + 1] == -1 && board[pos + 2] == -1) {
								// 空白
								if (put4(pos + 1) == 1) {
									box[turns - 1] = pos + 1;
									return (pos + 1); // 三目置く
					...
					if ((pos + 1) < board_size * board_size && (pos + 1) % board_size != 0) { // 右
						if (((pos - 1) > -1 && (pos - 1) % board_size != (board_size - 1))) { // 三目狙いどちらか1つ
					...
					if ((pos - 1) > -1 && (pos - 1) % board_size != (board_size - 1)) { // 左
						if (((pos - 2) > -1 && (pos - 2) % board_size != (board_size - 1))) { // 三目狙いどちらか1つ
							if (board[pos - 1] == -1 && board[pos - 2] == -1) {
								// 空白
								if (put4(pos - 1) == 1) {
									box[turns - 1] = pos - 1;
					...
					if ((pos - 1) > -1 && (pos - 1) % board_size != (board_size - 1)) { // 左
						if (((pos + 1) < board_size * board_size && (pos + 1) % board_size != 0)) { // 三目狙いどちらか1つ
							if (board[pos - 1] == -1 && board[pos + 1] == -1) {
								// 空白
								if (put4(pos - 1) == 1) {
									box[turns - 1] = pos - 1;
					...
					if ((pos + board_size) < board_size * board_size) { // 下
						if (((pos + (2 * board_size)) < board_size * board_size)) { // 三目狙いどちらか一つ
							if (board[pos + board_size] == -1 && board[pos + (2 * board_size)] == -1) { // 空白
								if (put4(pos + board_size) == 1) {
									box[turns - 1] = pos + board_size;
									return (pos + board_size); // 三目置く
								}
					...
					if ((pos + board_size) < board_size * board_size) { // 下
						if (((pos - board_size) > -1)) { // 三目狙いどちらか一つ
							if (board[pos + board_size] == -1 && board[pos - board_size] == -1) { // 空白
								if (put4(pos + board_size) == 1) {
									box[turns - 1] = pos + board_size;
									return (pos + board_size); // 三目置く
								}
					...
					if ((pos - board_size) > -1) { // 上
						if ((pos + board_size < board_size * board_size)) { // 三目狙いどちらか1つ koko
							if (board[pos - board_size] == -1 && board[pos + board_size] == -1) { // 空白
								if (put4(pos - board_size) == 1) {
									box[turns - 1] = pos - board_size;
									return (pos - board_size); // 三目置く
					...
					if ((pos - board_size) > -1) { // 上
						if ((pos - (2 * board_size) > -1)) { // 三目狙いどちらか1つ
							if (board[pos - board_size] == -1 && board[pos - (2 * board_size)] == -1) { // 空白
								if (put4(pos - board_size) == 1) {
									box[turns - 1] = pos - board_size;
									return (pos - board_size); // 三目置く
